...
const menus =ref<IMenu[]> ([
    ...
    {title:'编辑器',icon:'fas fa-keyboard',children:[
        {title:'Markdown编辑器'},
        {title:'富文本编辑器'}
    ]},
]) 
...
<template>
<div class="menu w-[200px] h-full bg-gray-800 p-4">
         ...
         <div class="left-container mt-2">
             <dl v-for="(menu, index) of menus" :key="index" @click="handle(menu,menu.children)">
                 ...
                 <dd v-if="menu.active" :class="{active:cmenu.active}" v-for="(cmenu,key) of menu.children" :key="key">{{cmenu.title}}</dd>
             </dl>
         </div>
     </div>
</template>
...
.left-container {
    dl {
        ...
        dd {
            @apply pl-4 py-2 my-2 text-white text-sm duration-300 rounded-sm cursor-pointer hover:bg-violet-500;
